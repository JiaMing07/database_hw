...
class CLRInsertLog : public LogRecord {
 public:
  CLRInsertLog(lsn_t lsn, xid_t xid, lsn_t prev_lsn, oid_t oid, pageid_t page_id, slotid_t slot_id, db_size_t page_offset,
            db_size_t record_size, char *record);
  ~CLRInsertLog();

  size_t SerializeTo(char *data) const override;
  static std::shared_ptr<CLRInsertLog> DeserializeFrom(lsn_t lsn, const char *data);
